package it.francescofiora.product.web.util;
...
public interface ResponseUtil {
  /**
   * Wrap the optional into a {@link org.springframework.http.ResponseEntity} with an
   * {@link org.springframework.http.HttpStatus#OK} status, or if it's empty, it returns a
   * {@link org.springframework.http.ResponseEntity} with
  ...
  /**
   * Wrap the optional into a {@link org.springframework.http.ResponseEntity} with an
   * {@link org.springframework.http.HttpStatus#OK} status with the headers, or if it's empty,
   * throws a {@link NotFoundAlertException} with status
   * {@link org.springframework.http.HttpStatus#NOT_FOUND}.
   *
   * @param <X> type of the response
   * @param maybeResponse response to return if present
   ...
   * @throws NotFoundAlertException {@code 404 (Not found)} if {@code maybeResponse} is empty
   */
  static <X> ResponseEntity<X> wrapOrNotFound(String entityName, Optional<X> maybeResponse,
      HttpHeaders header) {
