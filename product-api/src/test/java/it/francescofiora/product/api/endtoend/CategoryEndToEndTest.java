...
class CategoryEndToEndTest extends AbstractTestEndToEnd {
  ...
  void testCreate() {
    ...
    var categoryId = createAndReturnId(CATEGORIES_URI, newCategoryDto, ALERT_CREATED);

    final var categoryIdUri = String.format(CATEGORIES_ID_URI, categoryId);

    var categoryDto = TestUtils.createCategoryDto(categoryId);
    update(categoryIdUri, categoryDto, ALERT_UPDATED, String.valueOf(categoryId));

    var actual =
        get(categoryIdUri, CategoryDto.class, ALERT_GET, String.valueOf(categoryId));
    ...
    var categories = get(CATEGORIES_URI, PageRequest.of(1, 1), CategoryDto[].class,
    ...
    delete(categoryIdUri, ALERT_DELETED, String.valueOf(categoryId));

    assertGetNotFound(categoryIdUri, CategoryDto.class, ALERT_NOT_FOUND,
  ...
  void testGetBadRequest() {
    assertGetBadRequest(CATEGORIES_URI + "/999999999999999999999999", String.class,
  ...
  void testUpdateBadRequest() {
    // id
    assertUpdateBadRequest(String.format(CATEGORIES_ID_URI, 1L),
        TestUtils.createCategoryDto(null), ALERT_BAD_REQUEST, PARAM_ID_NOT_NULL);

    var id =
        createAndReturnId(CATEGORIES_URI, TestUtils.createNewCategoryDto(), ALERT_CREATED);

    assertUpdateBadRequest(String.format(CATEGORIES_ID_URI, (id + 1)),
    ...
    assertUpdateBadRequest(path, categoryDto, ALERT_BAD_REQUEST, PARAM_NAME_NOT_BLANK);
    ...
    assertUpdateBadRequest(path, categoryDto, ALERT_BAD_REQUEST,
